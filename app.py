...
if not df_raw.empty
    ...
    table_countries = st.multiselect(Seleziona paesei per la tabella, countries_options, default=[All])

    all_sources = sorted(df[Source].unique())
    sources_options = [All] + all_sources
    table_sources = st.multiselect(Seleziona fontee per la tabella, sources_options, default=[All])
    ...
    table_years = st.multiselect(Seleziona annoi per la tabella, years_options, default=[All])
    ...
    if df_graph.empty
        st.warning(fNessun dato disponibile per {graph_country}.)
    else
        ...
        ax.set_title(fQuota di Generazione - {graph_country})
        ax.set_ylabel('%')
        ax.set_ylim(0, 100)
        ax.set_xlabel('Anno')
